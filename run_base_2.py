...
class GaussianTrainer:
    ...
    def _build_or_load_fixed_eval_set(self) -> List[Path]:
        ...
        - Uses --test_glob or --test_dir discovery (sorted).
        ...
        if fixed_path.exists():
            ...
            if len(paths) > 0:
                ...
                return paths  # deterministic reuse

        # (re)discover now; FULL directory set, sorted
        candidates = self._discover_test_images()  # already sorted in your helper
        if len(candidates) == 0:
            raise FileNotFoundError(
                "No test images found. Provide --test_glob or --test_dir (or place test images "
                "under your dataset path)."
            )
    ...
    def _phaseB_eval_single(self, base_state_dict, img_path: Path, opt_iters: int,
        ...
        # load dataset artifacts (global PCA)
        with open(self.dataset_path / "pca_object.pkl", "rb") as FIN:
            pca_object = pickle.load(FIN)
        with open(self.dataset_path / "norm_infos.pkl", "rb") as FIN:
            norm_infos = pickle.load(FIN)
        ...
        with torch.no_grad():
            ...
            ssim0 = ms_ssim(img0.unsqueeze(0), gt_img.unsqueeze(0),
        ...
        if save_dir is not None:
            ...
            stem = img_path.stem
            # save GT, INIT
            gt_rgb = self._ycbcr_to_rgb_image(gt_img)
            init_rgb = self._ycbcr_to_rgb_image(img0)
            gt_rgb.save(save_dir / f"{stem}_gt.png")
            init_rgb.save(save_dir / f"{stem}_init.png")
        ...
        with torch.no_grad():
            ...
            ssimF = ms_ssim(imgF.unsqueeze(0), gt_img.unsqueeze(0),
                            data_range=1, size_average=True).item()
        ...
        return {
            ...
            "rgb_gt": rgb_gt, "rgb_init": rgb_init, "rgb_final": rgb_final,
        }

    # === NEW === public: evaluate on K test images during phase-A
    def eval_phaseB_on_test_set(self, global_iter: int):
        """
        Deterministic eval on the entire test directory (or glob).
        - Fixed set saved to model_dir/eval_set.json
        - Saves GT/INIT/FINAL PNGs under: {model_dir}/eval/iter_{iter:06d}/
        - Logs per-image + mean PSNR/SSIM to W&B with a table of images.
        """
        if self.wandb_run is None:
            # still run + save locally even if W&B is off
            pass

        # fixed, deterministic list (sorted once and saved)
        ...
        # where to save images for this eval
        ...
        # W&B artifacts (optional but recommended)
        if self.wandb_run:
            # images + metrics table
            table = wandb.Table(
                data=wandb_rows,
                columns=[
                    "path", "GT", "INIT", "FINAL",
                    "psnr_init", "ssim_init", "psnr_final", "ssim_final"
                ]
            )
            wandb.log({
                "iter": global_iter,
