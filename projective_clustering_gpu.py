...
@torch.no_grad()
def null_space_torch(X: torch.Tensor, rtol: float | None = None) -> torch.Tensor:
    """
    Orthonormal basis for the right nullspace of X (columns span N(X)).
    Returns N with shape (d, d - rank(X)) for X of shape (J, d).
    """
    U, S, Vh = torch.linalg.svd(X, full_matrices=False)
    if S.numel() == 0:  # degenerate
        return torch.eye(X.shape[1], device=X.device, dtype=X.dtype)
    if rtol is None:
        rtol = torch.finfo(X.dtype).eps * max(X.shape)
    rank = int((S > rtol * S.max()).sum().item())
    return Vh[rank:].T.contiguous()  # (d, q)

# @torch.no_grad()
# def computeDistanceToSubspace(point: torch.Tensor, X: torch.Tensor, v: torch.Tensor | None = None) -> torch.Tensor:
#     """
#     GPU version of:
#         if point.ndim > 1:
#             return || (point - v[None,:]) @ null_space(X) ||_2 along axis=1
#         else:
#             return || (point - v) @ null_space(X) ||_2
#     """
#     # align dtype/device to X (like your original expects consistent arrays)
#     dev, dt = X.device, X.dtype
#     point = point.to(device=dev, dtype=dt)
#     v = (v.to(device=dev, dtype=dt) if v is not None else None)

#     N = null_space_torch(X)  # (d, q), q may be 0

#     if point.ndim > 1:
#         R = point - (v.unsqueeze(0) if v is not None else 0.0)
#         return torch.linalg.norm(R @ N, dim=1)
#     else:
#         r = point - v if v is not None else point
#         return torch.linalg.norm(r @ N)
...
#     Prints per-step diagnostics:
#       - total objective value
#       - cluster sizes
#       - mean distance per cluster
#       - number of points that changed assignment since previous step
#       - points moved in/out per cluster
#     """
#     start_time = time.time()
#     n, d = P.shape
#     _ = torch.linalg.norm(P, dim=1).max()  # parity with original
...
#     for init_id in range(NUM_INIT_FOR_EM):  # run EM for 10 random initializations
#         # == init (same logic as original) ==
#         perm = torch.randperm(n, device=P.device)
#         vs = P[perm[:k], :].clone()                      # (k, d)
...
#             Vs[i].zero_()
#             Vs[i, :J_use, :] = Vi
#             vs[i, :] = vi                                 # <-- fix: set the affine offset for this cluster
...
#                 f"mean_dists={[round(x,4) for x in mean_per_cluster]} | "
...
def EMLikeAlg(PY: torch.Tensor, PCB: torch.Tensor, PCR: torch.Tensor,
    ...
    Inputs:
      PY, PCB, PCR: (N,d) Y/Cb/Cr feature matrices (float)
      w: (N,) weights
      j: subspace dim per channel
      k: number of clusters
      steps: EM iterations
      alphas: (αY, αCb, αCr) channel weights in the joint cost

    Returns:
      Vs: (K, 3, j, d)  row bases per (cluster, channel)
      vs: (K, 3, d)     means per (cluster, channel)
      elapsed_sec: float
    """
    start_time = time.time()
    N, d = PY.shape
    ...
    for init_id in range(NUM_INIT_FOR_EM):
        ...
        # ---- end init: evaluate final joint objective (FIX: per-cluster mask; no Vs[assign,...]) ----
    ...
    return best_Vs, best_vs, (time.time() - start_time)# @torch.no_grad()
